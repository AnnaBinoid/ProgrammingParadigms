...
// бот всегда ставит о
// бот всегда выбирает случайную пустую клетку
// без ООП (чистый процедурный)
// индексация рядов и колонок начинается с 0

// 1. Сначала набрасывем структуру
// 2. Функции начинаем с глагола
// 3. Пишем код маленькими короткими кусочками и проверяем (дебагер, вызов, тесты)
// 4. Порядок разработки - что где используем. Здесь createBoard -> inputCellCoordinates ->
//     -> makePlayerTurn -> startGameLoop -> getRandomEmptyCellCoordinates ->
//     -> makeBotTurn -> checkGameState
// 5. Когда создавать глобальную переменную (см. Сканнер), когда - нет (board). Если мы
//    играем несколько раундов - сканнер всегда один и тот же. Доска каждый раз создается
//    заново в нужном нам месте.
...
public class Main {
    private static int TABLE_SIZE = 3; // большими буквами через _ - когда переменная глобальная константа
...
//        inputCellCoordinates(board);
    ...
    public static void startGameRound() {
        ...
        String[][] board = createBoard(); // создали поле, положили его в board
        startGameloop(board);
        // startGameLoop

    }

    public static String[][] createBoard(){ // проверена
    ...
    public static void startGameloop(String[][] board) {
        ...
        // while (gameNotOver)
        // playerTurn (клетка не занята, не выходит за границы поля и тд)
        // botTurn
        // checkGameState (Х_WIN, O_WIN, DRAW, GAME_NOT_OVER_
    }
    ...
    public static int[] inputCellCoordinates(String[][] board){ // пишем функцию, которая будет принимать 2 координаты через пробел
    ...
    public static String checkGameState(String[][] board) { // проверка кто выиграл
...
/*       куча ифов - китайский код. Логика упрощения:
         x == 1, o == (-1), empty == 0
